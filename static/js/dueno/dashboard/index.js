...
const DASHBOARD_DETAIL_PREFIX = "/dueno/vehiculos";
const DASHBOARD_TOUR_STORAGE_KEY = "tectrack_dashboard_tour_completed";

let dashboardTourDriver = null;
let dashboardTourProgramado = false;
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    configurarEventosTourDashboard();
...
function configurarEventosTourDashboard() {
    document
        .getElementById("btnVerRecorridoDashboard")
        ?.addEventListener("click", () => {
    ...
    window.TecTrackSecurityDashboardTour = {
        iniciar: () => iniciarTourDashboard({
            forzado: true
        }),
        reiniciar: () => reiniciarTourDashboard(),
        completar: () => marcarTourDashboardComoCompletado()
    };
}

function programarTourDashboardInicial() {
    if (dashboardTourProgramado) return;
    if (tourDashboardEstaCompletado()) return;

    dashboardTourProgramado = true;

    window.setTimeout(() => {
        if (!tourDashboardEstaCompletado()) {
            iniciarTourDashboard();
        }
    }, 350);
}

function iniciarTourDashboard({ forzado = false } = {}) {
    if (!forzado && tourDashboardEstaCompletado()) {
        return;
    }

    if (dashboardTourDriver?.isActive?.()) {
        return;
    }

    const driverObj = obtenerDriverTourDashboard();

    if (!driverObj) {
        return;
    }

    driverObj.setSteps(construirPasosTourDashboard());
    driverObj.drive(0);
}

function reiniciarTourDashboard() {
    limpiarEstadoTourDashboard();
    dashboardTourProgramado = false;

    if (dashboardTourDriver?.isActive?.()) {
        dashboardTourDriver.destroy();
    }

    dashboardTourDriver = null;
}

function obtenerDriverTourDashboard() {
    if (dashboardTourDriver) {
        return dashboardTourDriver;
    }

    const driverFactory = window.driver?.js?.driver;

    if (typeof driverFactory !== "function") {
        console.warn("Driver.js no está disponible para el tour del dashboard.");
        return null;
    }

    const pasos = construirPasosTourDashboard();

    if (!pasos.length) {
        console.warn("No se encontraron elementos suficientes para iniciar el tour del dashboard.");
        return null;
    }

    dashboardTourDriver = driverFactory({
        animate: true,
        overlayColor: "rgba(15, 23, 42, 0.72)",
        popoverClass: "driverjs-tectrack-tour",
        popoverOffset: 14,
        showProgress: true,
        ...
        onCloseClick: () => finalizarTourDashboard(),
        onDoneClick: () => finalizarTourDashboard(),
        onPopoverRender: (popover, { state }) => {
            personalizarPopoverTourDashboard(popover, state);
        },
        steps: pasos
    });
...
function construirPasosTourDashboard() {
    const pasos = [];
    ...
    configuracionPasos.forEach((paso) => {
    ...
    pasos.unshift({
        popover: {
            title: "Bienvenido a TecTrack Security",
            description: "Te mostraremos rápidamente las partes más importantes del dashboard para que te ubiques desde el inicio.",
            showButtons: ["next"]
        }
    });
...
function finalizarTourDashboard() {
    marcarTourDashboardComoCompletado();
...
function marcarTourDashboardComoCompletado() {
    try {
        // Guardamos un flag simple para evitar que el tour se abra
        // automáticamente en futuras visitas.
        localStorage.setItem(
            DASHBOARD_TOUR_STORAGE_KEY,
            "1"
        );
    } catch (error) {
        console.warn("No se pudo guardar el estado del tour del dashboard.", error);
    }
}

function limpiarEstadoTourDashboard() {
    try {
        // Útil para pruebas: borra el estado guardado y hace que el tour
        // vuelva a mostrarse como primera visita.
        localStorage.removeItem(DASHBOARD_TOUR_STORAGE_KEY);
    } catch (error) {
        console.warn("No se pudo limpiar el estado del tour del dashboard.", error);
    }
}

function tourDashboardEstaCompletado() {
    try {
        // Leemos el estado persistido para decidir si el tour debe abrirse
        // de forma automática.
        return localStorage.getItem(DASHBOARD_TOUR_STORAGE_KEY) === "1";
    } catch (error) {
        console.warn("No se pudo leer el estado del tour del dashboard.", error);
        return false;
    }
}
